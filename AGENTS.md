...
**Recently Completed:** Newsletter signup — `NewsletterForm.astro` at the end of `PostLayout.astro` and in `Footer.astro` on every page *except the homepage* (human wants it kept bare), posting straight to Buttondown's public embed-subscribe endpoint (no API key, no backend — a deliberate architecture call the human approved, see `MEMORY.md`). Footer heading is section-aware (different copy on Writing/Projects/About+Now) and warmer in tone, per the human's explicit direction. `rss.xml.ts` now carries full post content (needed for Buttondown's optional RSS-to-email automation, which the human is holding off on — $9/mo add-on). `privacy.astro` updated to describe what Buttondown collects. Verified in a real browser each time (form present/absent correctly per page, correct copy per section, no console/CSP errors, `npx astro check` and `npm run build` both clean throughout). Before that: security pass (`scripts/generate-headers.mjs` generates build-time CSP/security headers, Dependabot enabled) and an SEO pass (canonical URLs, OpenGraph/Twitter cards, sitemap, robots.txt). See `MEMORY.md` for full detail on all of this.
