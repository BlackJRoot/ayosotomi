...
### Phase 3 item 5 (part 1) — mobile sweep + accessibility basics

**What it is:** A systematic 375px audit of all 15 routes plus baseline accessibility fixes. (The performance/Lighthouse-number half is separate — needs a real Chrome run.)

**Concepts learned:**
- **A whole-site mobile sweep catches things per-feature checks miss — but here it confirmed the foundation was already solid.** Every one of the 15 routes had zero horizontal overflow at 375px. That's a payoff from checking mobile after each feature all along, plus one structural choice: the `<pre>` code block scrolls internally (`overflow-x: auto`, contained within the viewport) rather than pushing the page wide, and images cap at the content width. The fanned-deck project cards *do* extend a few px past the card edge by design (their "peek"), but stay within the viewport — not overflow.
- **Heading hierarchy is content, not just layout.** The audit flagged an H1→H3 skip on one essay. Root cause: that essay's markdown used `###` for its two section headings while the post title is H1 (from the layout) and every other post correctly uses `##`. The extra headings the audit saw (an H2, then H3s) were the layout's own "Related" section (H2) and the related-post `PostCard` titles (H3) — those are fine; only the body's `###`-before-any-`##` was the real skip. Fix was a two-character content edit (`###` → `##`). Lesson: when auditing headings, separate *layout-injected* headings (title, "Related", card titles) from *content* headings.
- **The canonical skip-link pattern beats Tailwind's `sr-only` + `focus:not-sr-only` dance.** First attempt combined `sr-only` with `focus:not-sr-only` and a pile of layout utilities on one `<a>` — it produced a broken in-between state (stuck at 34×18, neither hidden nor revealed) because those utilities fight each other in Tailwind v4's cascade. Replaced with a dedicated `.skip-link` class in `global.css` that parks the link off-screen (`top: -4rem`) and slides it to `top: 1rem` on `:focus`. Simple, reliable, and it's the classic implementation for a reason.
- **`:focus`/`:focus-visible`/`:visited` are all unverifiable via programmatic focus in the headless pane — but for a consistent reason.** `document.activeElement` was the skip link, yet `element.matches(':focus')` returned `false`: the browser window doesn't hold system focus, so CSS focus pseudo-classes never activate even when JS focus is set. Same root cause as the earlier `:visited` limitation. Worked around it by proving the *positioning* directly — force `top: 1rem` (with the transition temporarily disabled to avoid measuring mid-animation) and confirm the link lands on-screen at 133×39px — leaving only the trigger itself (a standard, correct CSS rule) unverified by automation.
- **Disable transitions before synchronously measuring a state change.** Repeatedly got misleading reads (element "still at −64" right after changing `top`) because `transition: top 0.2s` meant `getBoundingClientRect` caught the value mid-animation. Setting `transition: none` + forcing a reflow before measuring gives the true settled value.

**Problems encountered + solutions:**
1. Fixed the one heading-hierarchy skip (quantum essay `###` → `##`); confirmed body headings are now `H2,H2` under the H1 title.
2. Added a skip-to-content link (`.skip-link` → `#main`) and palette-tuned `:focus-visible` outline styles (2px accent outline, adapts to both themes via `--color-accent`). `<main>` got an `id="main"` target.
3. Baseline a11y confirmed already-good: `lang="en"`, `<main>` landmark, all prose images have alt text, no CSS suppressing focus outlines, filter buttons 30px / prev-next links 44px tap height. One minor item left as-is: header/footer *text* links are ~20px tall (inline links, exempt from tap-target minimums, adequately spaced) — not worth disrupting the just-approved header layout.
4. Not done here (needs the human's Chrome): the actual Lighthouse Performance/Accessibility 90+ score. `astro check` 0 errors/23 files, 15-page build clean.

### Content cleanup — separating AI-authored placeholders from real reference material

**What it is:** The human asked to draw a hard line between their real content (the 4 migrated Substack essays) and everything I'd authored as sample/filler content across project-logs, tutorials, projects, and Now — removing the latter and replacing each with a clearly-labeled, fully-worked reference example instead, plus a written guide on how the index pages work.

**Concepts learned:**
- **"Fully worked example" and "hidden from visitors" are two different, orthogonal requirements — and the schema decides whether both are possible.** For the `blog` collection (project-logs, tutorials), `draft: true` cleanly satisfies "hidden but present in source." For `projects`, there's no `draft` field — a decision the human explicitly declined to change when asked directly ("No. I do not approve. Skip this part."). The honest resolution wasn't to work around that with a hack (e.g. an unused status value, a naming convention) — it was to accept the constraint and make the *visible* placeholder obviously a placeholder: `"[Template]"` in the title, an explanatory description, and an HTML comment spelling out exactly why it's public and what to do about it.
- **A schema extension can be additive and fully backward-compatible.** Adding `watching`/`doing` to the `now` schema as `.optional()` fields (capped at `.max(3)`, smaller than the required fields' `.max(5)`, reflecting their lighter "extra" status) meant zero risk to the existing single `now` entry or the schema's required shape — Zod's `.optional()` means old content missing those fields still validates. The page then explicitly filters to only show them when present, keeping the "extras" both structurally distinct (own schema fields) and visually distinct (one condensed line, not full sections) from the four core fields.
- **A "skip-placeholder" filter needs to know about every placeholder convention actually in use — checking this surfaced a real bug.** The homepage's `firstReal()` helper (built earlier to hide the em-dash-prefixed `"— add what you're reading —"` convention) didn't know about the *new* Now template's `[bracketed]` placeholder convention introduced by this task — so without a fix, a freshly-templated Now entry would have leaked literal placeholder text like `"[what you're currently building or focused on]"` onto the live homepage pulse. Two different parts of the same codebase (the content template's placeholder style, and the homepage's placeholder-detection logic) were written far apart and needed to agree on convention; nothing forced that agreement automatically. Fixed by extending the filter to recognize both conventions.
- **The actual `/now` page and the homepage's "Currently" pulse are supposed to behave differently for placeholder content, on purpose.** `/now` is the living document — it should show exactly what's written, including a raw unfilled template, since that's useful signal to the page's one intended reader (the human, deciding what to fill in). The homepage pulse is a curated "vibe" line for visitors — it should never show a raw placeholder. Same underlying data, deliberately different display rules per page's purpose.
- **Verifying a large batch of changes benefits from checking *what's actually on disk* before assuming a plan needs executing.** Content compaction earlier in this long session meant the actual implementation of this task (schema, template, page, and reference-post files) had already happened, but wasn't visible in recent conversation history. Rather than blindly re-implementing (risking duplicate/conflicting work) or blindly trusting a system note claiming files changed, independently re-read every file via the filesystem before deciding what, if anything, still needed doing — which surfaced that only the placeholder-filter bug above actually needed a fix.

**Problems encountered + solutions:**
1. Fixed the `firstReal()` bracket-placeholder gap described above; verified via the browser that the homepage's "Currently" section now correctly disappears entirely when every Now field is still template placeholder text (rather than leaking `[bracketed]` text).
2. Verified end-to-end: `/now` shows the raw template (including the condensed `Watching: ... · Doing: ...` line) as intended; `/writing` correctly lists exactly the 4 real essays with only `all`/`essay` filter buttons (tutorial/project-log currently have zero live posts); `/projects` shows the honestly-labeled `[Template]` entry with no overflow. `astro check` 0 errors/23 files, 12-page build (down from 15 — reflects the two drafted templates and removed real content), no console errors.
